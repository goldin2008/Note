...
subprocess.check_call(
    [sys.executable, "-m", "pip", "install", "transformers==3.5.1"]
)
...
def process(args):
    ...
    for file in input_files:
        ...
        df_temp = pd.read_csv(
            file_path, sep="\t", usecols=["sentiment", "review_body"]
        )
    ...
    plot_conf_mat(
        cm, classes=classes, title="Confusion Matrix", cmap=plt.cm.Greens
    )
