...
def parse_args():
    ...
    parser.add_argument("--train_steps_per_epoch", type=int, default=64)
    ...
    parser.add_argument(
        "--output_dir", type=str, default=os.environ["SM_OUTPUT_DIR"]
    )
...
def create_data_loader(path, batch_size):
    ...
    for file in input_files:
        df_temp = pd.read_csv(
            file, sep="\t", usecols=["input_ids", "label_id"]
        )
...
def train_model(
    model, train_data_loader, df_train, val_data_loader, df_val, args
):
        ...
        for i, (sent, label) in enumerate(train_data_loader):
            if i < args.train_steps_per_epoch:
                ...
                if (
                    ...
                    print(
                        "[epoch/step: {0}/{1}] val_loss: {2:.2f} - val_acc: {3:.2f}%".format(
...
if __name__ == "__main__":
    ...
    while retries < 5 and not successful_download:
        try:
            ...
            print(
                "Retry #{}.  Sleeping for {} seconds".format(
